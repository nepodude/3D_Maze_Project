 ...
 * rectangles for wall texture mapping.
 * @i: The column index (ray).
 * @srcRect: Pointer to the source SDL_Rect for the texture.
 * @destRect: Pointer to the destination SDL_Rect on screen.
 */
void processRay(int i, SDL_Rect *srcRect, SDL_Rect *destRect)
{
	...
	/* Apply texture brightness modulation:
	 * North-South walls (closer to horizontal boundary) are darker.
	 */
	if (distToVertical < distToHorizontal)
...
/*
 * renderWalls - Render wall slices for all rays.
